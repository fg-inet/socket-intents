...
void print_policy_info(void *policy_info)
{
	...
	printf("\t(for filesize %6d =< n =< %6d), id: %d", info->minfilesize, info->maxfilesize, info->id);
	if (info->is_default)
		printf(" (default)");
}
...
void set_policy_info(gpointer elem, gpointer data)
{
	...
	if (spl->policy_set_dict != NULL)
	{
        ...
        if ((value = g_hash_table_lookup(spl->policy_set_dict, "id")) != NULL)
			new->id = atoi(value);
	}
...
static void set_sa_for_filesize(request_context_t *rctx, int filesize, strbuf_t sb)
{
	...
	while (elem != NULL)
	{
        ...
        {
            /* Filesizes do not fall within this prefixes' conf.: ignore */
			printf("DEBUG: ignoring prefix\n");
